...
class Test(unittest.TestCase):
    ...
    def test_alig_without_momentum(self):
        optimizer_tf = ALIG_tf(max_step_size=self.max_step_size)
        optimizer_th = ALIG_th(self.model_th.parameters(), eta=self.max_step_size)

        self.train_with(optimizer_tf, optimizer_th)

    def test_alig_with_momentum(self):
        optimizer_tf = ALIG_tf(max_step_size=self.max_step_size, momentum=self.momentum)
        optimizer_th = ALIG_th(self.model_th.parameters(), eta=self.max_step_size,
                               momentum=self.momentum, adjusted_momentum=True)
